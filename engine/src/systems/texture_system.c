...
b8 texture_system_initialize(
    u64* memory_requirement, void* state, texture_system_config config
) {
    ...
    *memory_requirement =
    ...
    invalid_ref.handle =
        INVALID_ID;  // Primary reason for needing default values.
...
texture* texture_system_acquire(const char* name, b8 auto_release) {
    // Return default texture, but warn about it since this should be returned
    // via get_default_texture();
    ...
    if (state_ptr &&
        hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
        ...
        if (ref.handle == INVALID_ID) {
            ...
            if (!load_texture(name, t)) {
                OKO_ERROR("Failed to load texture '%s'.", name);
                return 0;
            }
    ...
    // NOTE: This would only happen in the event something went wrong with the
    // state.
    OKO_ERROR(
...
void texture_system_release(const char* name) {
    ...
    if (state_ptr &&
        hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
        if (ref.reference_count == 0) {
            OKO_WARN("Tried to release non-existent texture: '%s'", name);
            return;
        }
        ref.reference_count--;
        ...
        OKO_ERROR(
            "texture_system_release failed to release texture '%s'.", name
        );
    }
}
