...
// PRIVATE
void create(
    vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain
) {
    //
    ...
    swapchain_extent.width =
        OKO_CLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height =
        OKO_CLAMP(swapchain_extent.height, min.height, max.height);
    ...
    swapchain_create_info.compositeAlpha =
        VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;  // composite with os off
    ...
    if (!swapchain->images) {
        swapchain->images = (VkImage*)memory_allocate(
            sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER
        );
    }
    if (!swapchain->views) {
        swapchain->views = (VkImageView*)memory_allocate(
    ...
    if (!vulkan_device_detect_depth_format(&context->device)) {
        ...
        OKO_FATAL("Failed to find a supported format!");
    }
...
void destroy(vulkan_context* context, vulkan_swapchain* swapchain) {
    ...
    vkDestroySwapchainKHR(
        context->device.logical_device, swapchain->handle, context->allocator
    );
}
...
void vulkan_swapchain_destroy(
    vulkan_context* context, vulkan_swapchain* swapchain
) {
    //
    destroy(context, swapchain);
}
...
b8 vulkan_swapchain_acquire_next_image_index(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        OKO_FATAL("Failed to acquire swapchain image!");
        return false;
    }
...
void vulkan_swapchain_present(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        // Swapchain is out of data, suboptimal or a framebuffer resize has
        ...
        OKO_FATAL("Failed to present swapchain image!");
    }
