...
void vulkan_image_create(
    ...
    image_create_info.extent.depth = 1;  // TODO: Configurable depth
    image_create_info.mipLevels = 4;     // TODO: Mip mapping support
    image_create_info.arrayLayers = 1;   // TODO: Configurable image layer count
    ...
    if (memory_type == -1) {
        OKO_ERROR("Required memory type not found. Image not valid.");
    }
...
void vulkan_image_view_create(
    ...
    view_create_info.viewType =
        VK_IMAGE_VIEW_TYPE_2D;  // TODO: Configurable image view type
...
void vulkan_image_transition_layout(
    ...
    if (old_layout == VK_IMAGE_LAYOUT_UNDEFINED &&
        ...
        OKO_FATAL("Unsupported layout transition.");
        return;
    }
...
void vulkan_image_destroy(vulkan_context* context, vulkan_image* image) {
    ...
    if (image->view) {
        vkDestroyImageView(
            context->device.logical_device, image->view, context->allocator
        );
    ...
    if (image->memory) {
        vkFreeMemory(
            context->device.logical_device, image->memory, context->allocator
        );
    ...
    if (image->handle) {
        vkDestroyImage(
            context->device.logical_device, image->handle, context->allocator
        );
