...
b8 vulkan_material_shader_create(
    vulkan_context* context, vulkan_material_shader* out_shader
) {
    ...
    // Global descriptor pool: Used for global items such as view/projection
    // matrix.
...
#define ATTRIBUTE_COUNT 2
    ...
    if (!vulkan_graphics_pipeline_create(
        ...
        OKO_ERROR("Failed to load graphics pipeline for object shader.");
        return false;
    }
    ...
    if (!vulkan_buffer_create(
        ...
        OKO_ERROR("Vulkan buffer creation failed for object shader.");
        return false;
    }
    ...
    if (!vulkan_buffer_create(
            context,
            sizeof(object_uniform_object),  //* MAX_OBJECT_INSTANCE_COUNT,
            VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT |
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
        ...
        OKO_ERROR("Material instance buffer creation failed for shader.");
        return false;
    }
...
void vulkan_material_shader_update_object(
    ...
    f32 s = (oko_sin(accumulator) + 1.0f) / 2.0f;  // scale from -1, 1 to 0, 1
    obo.diffuse_color = vec4_create(s, s, s, 1.0f);

    // Load the data into the buffer.
    vulkan_buffer_load_data(
        context, &shader->object_uniform_buffer, offset, range, 0, &obo
    );
...
b8 vulkan_material_shader_acquire_resources(
    ...
    *out_object_id = shader->object_uniform_buffer_index;
    ...
    if (result != VK_SUCCESS) {
        OKO_ERROR("Error allocating descriptor sets in shader!");
        return false;
    }
...
void vulkan_material_shader_release_resources(
    ...
    if (result != VK_SUCCESS) {
        OKO_ERROR("Error freeing object shader descriptor sets!");
    }
