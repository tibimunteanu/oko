...
u64 hash_name(const char* name, u32 element_count) {
    // a multiplier to use when generating a hash. prime to hopefully avoid
    // collisions.
...
void hashtable_create(
    ...
    if (!memory || !out_hashtable) {
        OKO_ERROR(
            "hashtable_create failed! Pointer to memory and out_hashtable are required."
        );
        return;
    }
    ...
    // TODO: Might want to require an allocator and allocate this memory
    // instead.
    ...
    memory_zero(out_hashtable->memory, element_size * element_count);
}
...
b8 hashtable_set(hashtable* table, const char* name, void* value) {
    if (!table || !name || !value) {
        OKO_ERROR("hashtable_set requires table, name and value to exist.");
        return false;
    }
    if (table->is_pointer_type) {
        OKO_ERROR(
            "hashtable_set should not be used with tables that have pointer types. Use hashtable_set_ptr instead."
        );
        return false;
    }
...
b8 hashtable_set_ptr(hashtable* table, const char* name, void** value) {
    if (!table || !name) {
        OKO_WARN("hashtable_set_ptr requires table and name  to exist.");
        return false;
    }
    if (!table->is_pointer_type) {
        OKO_ERROR(
            "hashtable_set_ptr should not be used with tables that do not have pointer types. Use hashtable_set instead."
        );
        return false;
    }
...
b8 hashtable_get(hashtable* table, const char* name, void* out_value) {
    if (!table || !name || !out_value) {
        OKO_WARN("hashtable_get requires table, name and out_value to exist.");
        return false;
    }
    if (table->is_pointer_type) {
        OKO_ERROR(
            "hashtable_get should not be used with tables that have pointer types. Use hashtable_set_ptr instead."
        );
        return false;
    }
    u64 hash = hash_name(name, table->element_count);
...
b8 hashtable_get_ptr(hashtable* table, const char* name, void** out_value) {
    ...
    if (!table->is_pointer_type) {
        OKO_ERROR(
            "hashtable_get_ptr should not be used with tables that do not have pointer types. Use hashtable_get instead."
        );
        return false;
    }
...
b8 hashtable_fill(hashtable* table, void* value) {
    if (!table || !value) {
        OKO_WARN("hashtable_fill requires table and value to exist.");
        return false;
    }
    if (table->is_pointer_type) {
        OKO_ERROR(
            "hashtable_fill should not be used with tables that have pointer types."
        );
        return false;
    }
    ...
    for (u32 i = 0; i < table->element_count; ++i) {
        memory_copy(
            table->memory + (table->element_size * i),
            value,
            table->element_size
        );
    }
